...
export function ThreadSidebarItem({
  ...
  return (
    <div className={cn("relative", isNested ? "py-0.5" : "")}>
      <div className={cn("group flex items-start gap-2 rounded-md", isSelected ? "bg-surface-high" : "hover:bg-surface-high")}>
        <button
          className={cn("min-w-0 flex-1 rounded-md text-left outline-none transition-all focus-visible:ring-[3px] focus-visible:ring-accent/30 motion-reduce:transition-none", isNested ? "px-2 py-1" : "px-2.5 py-1.5")}
          onClick={onSelect}
          type="button"
        >
        ...
        <div className="relative shrink-0 px-1.5 pt-1.5">
          <button
            ...
            className={cn("rounded-md p-1 text-muted outline-none transition-colors hover:bg-surface-high hover:text-ink focus-visible:ring-[3px] focus-visible:ring-accent/30", menuOpen || renaming ? "opacity-100" : "opacity-0 group-hover:opacity-100 focus:opacity-100")}
            onClick={(event) => {
