...
import { Archive, CheckCircle2, Folder, LoaderCircle, MoreHorizontal, Pencil, Trash2 } from "lucide-react";
...
export function ThreadSidebarItem({
  ...
  return (
    <div className={cn("relative", isNested ? "py-0.5" : "")}>
      ...
      {renaming ? (
        <form
          className={cn("mt-2 rounded-xl border border-line bg-surface-high p-2 shadow-[var(--shadow-menu)]", isNested ? "ml-2" : "")}
        ...
        >
          ...
          <div className="mt-2 flex justify-end gap-2">
            <Button onClick={onRenameCancel} size="sm" type="button" variant="secondary">
              Cancel
            </Button>
            <Button disabled={!renameValue.trim()} size="sm" type="submit" variant="default">
              Save
            </Button>
          </div>
        </form>
